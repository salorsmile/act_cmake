...
int main(int argc, char* argv[]) {
	...
	std::shared_ptr<BMNNContext> bm_ctx_detector =
		std::make_shared<BMNNContext>(handle, bmodel_detector.c_str());
	...
	std::string save_result_path = "results/mot_eval/";
	std::ofstream mot_saver;
	...
	mot_saver.open(save_result_path, std::ios::out);
	bool end_flag = false;
	int ind = 0;
	while (!end_flag) {
		ts->save("decode time");
		...
		if ((batch_imgs.size() == batch_size || end_flag) && !batch_imgs.empty()) {
			...
			for (int i = 0; i < batch_imgs.size(); i++) {
				...
				bytetrack.update(output_stracks, yolov5_boxes[i]);
				ts->save("bytetrack time");

				ts->save("encode time");
				for (auto& track_box : output_stracks) {
					std::string save_str = cv::format(
						"%d,%d,%f,%f,%f,%f,1,-1,-1,-1\n", id, track_box->track_id,
						track_box->tlwh[0], track_box->tlwh[1], track_box->tlwh[2],
						track_box->tlwh[3]);
					mot_saver << save_str;
				}
				std::cout << id << ", detect_nums: " << yolov5_boxes[i].size()
					<< "; track_nums: " << output_stracks.size() << std::endl;

#if USE_OPENCV_DRAW_BOX
				cv::Mat frame_to_draw;
				cv::bmcv::toMAT(&batch_imgs[i], frame_to_draw);
				for (auto& bbox : output_stracks) {
					draw_opencv(bbox->track_id, bbox->class_id, bbox->score,
						bbox->tlwh[0], bbox->tlwh[1],
						bbox->tlwh[0] + bbox->tlwh[2],
